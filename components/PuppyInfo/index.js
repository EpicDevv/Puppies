...
const PuppyInfo = ({ puppyData }) => {
  ...
  if (
    !Array.isArray(puppyData.sliderData) ||
    puppyData.sliderData.length <= 0
  ) {
    return null;
  }
  return (
    <>
      <Head>
        <title>{puppyData.name} - {puppyData.gender} - {puppyData.available}</title>
        <meta name="robots" content="index, follow" />
        <meta name="description" content={puppyData.available} />
        <meta property="og:title" content={puppyData.name} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="" />
        <meta
          property="og:image"
          content="/puppypics/IMG_20221129_071245_184.jpg"
        />
        <meta
          property="og:description"
          content={puppyData.name}
        />
        <meta property="og:locale" content="en_US" />
        <meta property="og:site_name" content="SammyPups" />
        <meta name="description" content={puppyData.name} />
        <link rel="icon" href="/puppy.png" />
      </Head>
      <div className="min-h-screen justify-center items-center flex flex-col mt-[150px]">
        <article>
          <h1 className="text-center text-[50px]">{puppyData.name}</h1>
        ...
        <div className="text-center">
          Deposit Amount: $500 <br /> (you can pick puppy up anytime after
          deposit is made)
        </div>
        ...
        <div className="inline-flex bg-emerald-400 rounded-lg p-[20px] flex-col items-center justify-center mb-[100px] mt-[50px]">
          <div>
            <h1 className="text-center mb-3 underline">
              Call text or email us
            </h1>
            <a href="mailto: zayne.mngmt@gmail.com">
              {" "}
              <h1 className="mb-1 cursor-pointer bg-white px-[5px] rounded-lg">
